...
#              Messages Generated by Server-ACK_CHAT, CHAT_RESPONSE, ROUTING_INFO and JOINING_NETWORK_RELAY
#              Messages handled by Server-JOINING_NETWORK, CHAT, CHAT_RETRIVE, LEAVE_NETWORK, and PING
  ...
  def handleMsg(msgType, attributes)
    ...
    if msgType=="JOINING_NETWORK"
      ...
      handleLeaveNetwork(attributes)
  		...
  		@send.handleAckChat(attributes)
  	elsif msgType=="CHAT_RETRIVE"
  		handleRetriveMsg(attributes)
  	elsif msgType=="CHAT_RESPONSE"
  		      #sleep(6);
  		@send.chatResponseAck=true
  		puts "chatResponseAck= #{@send.chatResponseAck}"
  		@send.handleChatResponse(attributes)
  	elsif msgType=="PING"
  		handlePing(attributes)
  	elsif msgType=="ACK"
  		     #sleep(6);
  		@send.pingAck=true
  		@send.handlePingAck(attributes)
  	end
  end
  
  def handlePing(attributes)
     ip=getTheNodeToSend(attributes.fetch("target_id"))
     if ip==@host
     	puts "This node is the destination of ping\n"
     else
     	attributes["ip_address"]=ip  #change ip and forward to next hop
     	@send.sendMsg(attributes, ip)
     end
     sendPingAck(attributes, ip)
 end

 def sendPingAck(attributes, ip)
 	pingAck={
 		     type: "ACK",
 		     node_id:attributes.fetch("target_id"),
             ip_address:@host
 			}
 			puts pingAck
 			@send.sendMsg(pingAck, ip)
 end
  ...
  end                     #the only feature left to be implemented!!
